...
parser.add_argument("--jenis", default="Dublin", help="")
parser.add_argument("--area", default=1, help="")
parser.add_argument("--individu", default=False, help="")
...
if not FLAGS.individu:
    print("Mashook")
    for i in range(1, 7):
        if FLAGS.jenis == "RGB":
            file_name = "log_rgb_{}/output_filelist.txt".format(i)
        elif FLAGS.jenis == "Dublin":
            # file_name = 'log{}/output_filelist.txt'.format(i)
            file_name = "log_dublin_{}/output_filelist.txt".format(i)
        else:
            file_name = "log{}/output_filelist.txt".format(i)
        print(file_name)
    ...
    gt_label_filenames = [
        f.rstrip("_pred\.txt") + "_gt.txt" for f in pred_data_label_filenames
    ]
    print(gt_label_filenames)
else:
    print("Ra Mashook")
    if FLAGS.jenis == "RGB":
        file_name = f"log_rgb_{area}/output_filelist.txt"
    elif FLAGS.jenis == "Dublin":
        # file_name = 'log{}/output_filelist.txt'.format(i)
        file_name = f"log_dublin_{area}/output_filelist.txt"
    else:
        file_name = f"log{area}/output_filelist.txt"
    print(file_name)
    pred_data_label_filenames += [line.rstrip() for line in open(file_name)]
...
print("positive classes: ", positive_classes)
print("true positive: ", true_positive_classes)


print(
    "Overall accuracy: {0}".format(
